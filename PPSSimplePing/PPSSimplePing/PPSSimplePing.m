...
typedef struct PPSIPv4Header PPSIPv4Header;

__Check_Compile_Time(sizeof(PPSIPv4Header) == 20);
__Check_Compile_Time(offsetof(PPSIPv4Header, versionAndHeaderLength) == 0);
__Check_Compile_Time(offsetof(PPSIPv4Header, differentiatedServices) == 1);
__Check_Compile_Time(offsetof(PPSIPv4Header, totalLength) == 2);
__Check_Compile_Time(offsetof(PPSIPv4Header, identification) == 4);
__Check_Compile_Time(offsetof(PPSIPv4Header, flagsAndFragmentOffset) == 6);
__Check_Compile_Time(offsetof(PPSIPv4Header, timeToLive) == 8);
__Check_Compile_Time(offsetof(PPSIPv4Header, protocol) == 9);
__Check_Compile_Time(offsetof(PPSIPv4Header, headerChecksum) == 10);
__Check_Compile_Time(offsetof(PPSIPv4Header, sourceAddress) == 12);
__Check_Compile_Time(offsetof(PPSIPv4Header, destinationAddress) == 16);
...
static uint16_t PPS_in_cksum(const void *buffer, size_t bufferLen) {
    //
    size_t              bytesLeft;
    int32_t             sum;
...
- (void)didFailWithError:(NSError *)error {
    id<PPSSimplePingDelegate>  strongDelegate;
    ...
    if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(pps_simplePing:didFailWithError:)] ) {
        [strongDelegate pps_simplePing:self didFailWithError:error];
    }
}
...
- (NSData *)pingPacketWithType:(uint8_t)type payload:(NSData *)payload requiresChecksum:(BOOL)requiresChecksum {
    ...
    if (requiresChecksum) {
        ...
        icmpPtr->checksum = PPS_in_cksum(packet.bytes, packet.length);
    }
    
    return packet;
}

- (void)sendPacket:(NSData * _Nullable)data {
    int                     err;
    NSData *                payload;
    NSData *                packet;
    ssize_t                 bytesSent;
    ...
    switch (self.hostAddressFamily) {
        case AF_INET: {
            packet = [self pingPacketWithType:PPSICMPv4TypeEchoRequest payload:payload requiresChecksum:YES];
        } break;
        case AF_INET6: {
            packet = [self pingPacketWithType:PPSICMPv6TypeEchoRequest payload:payload requiresChecksum:NO];
        } break;
        default: {
            assert(NO);
        } break;
    ...
    if ( (bytesSent > 0) && (((NSUInteger) bytesSent) == packet.length) ) {
        ...
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(pps_simplePing:didSendPacket:sequenceNumber:)] ) {
            [strongDelegate pps_simplePing:self didSendPacket:packet sequenceNumber:self.nextSequenceNumber];
        }
    } else {
        NSError *   error;
        ...
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(pps_simplePing:didFailToSendPacket:sequenceNumber:error:)] ) {
            [strongDelegate pps_simplePing:self didFailToSendPacket:packet sequenceNumber:self.nextSequenceNumber error:error];
        }
    }
...
+ (NSUInteger)icmpHeaderOffsetInIPv4Packet:(NSData *)packet timeToLive:(NSInteger *)timeToLive {
    ...
    NSUInteger                  result;
    const struct PPSIPv4Header *   ipPtr;
    size_t                      ipHeaderLength;
    
    result = NSNotFound;
    if (packet.length >= (sizeof(PPSIPv4Header) + sizeof(PPSICMPHeader))) {
        ipPtr = (const PPSIPv4Header *) packet.bytes;
        if ( ((ipPtr->versionAndHeaderLength & 0xF0) == 0x40) &&            // IPv4
            ( ipPtr->protocol == IPPROTO_ICMP ) ) {
            ipHeaderLength = (ipPtr->versionAndHeaderLength & 0x0F) * sizeof(uint32_t);
            if (packet.length >= (ipHeaderLength + sizeof(PPSICMPHeader))) {
...
- (BOOL)validatePing4ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr timeToLive:(NSInteger *)timeToLive {
    ...
    if (icmpHeaderOffset != NSNotFound) {
        icmpPtr = (struct PPSICMPHeader *) (((uint8_t *) packet.mutableBytes) + icmpHeaderOffset);
        receivedChecksum   = icmpPtr->checksum;
        icmpPtr->checksum  = 0;
        calculatedChecksum = PPS_in_cksum(icmpPtr, packet.length - icmpHeaderOffset);
        icmpPtr->checksum  = receivedChecksum;
        
        if (receivedChecksum == calculatedChecksum) {
            if ( (icmpPtr->type == PPSICMPv4TypeEchoReply) && (icmpPtr->code == 0) ) {
...
- (BOOL)validatePing6ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    ...
    if (packet.length >= sizeof(*icmpPtr)) {
        ...
        if ( (icmpPtr->type == PPSICMPv6TypeEchoReply) && (icmpPtr->code == 0) ) {
            if ( OSSwapBigToHostInt16(icmpPtr->identifier) == self.identifier ) {
                uint16_t    sequenceNumber;
...
- (void)readData {
    ...
    if (bytesRead > 0) {
        ...
        if ( [self validatePingResponsePacket:packet sequenceNumber:&sequenceNumber timeToLive:&timeToLive] ) {
            if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(pps_simplePing:didReceivePingResponsePacket:timeToLive:sequenceNumber:timeElapsed:)] ) {
                NSTimeInterval timeElapsed = [[NSDate date] timeIntervalSinceDate:self.pingStartDate];
                [strongDelegate pps_simplePing:self didReceivePingResponsePacket:packet timeToLive:timeToLive sequenceNumber:sequenceNumber timeElapsed:timeElapsed];
            }
        } else {
            if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(pps_simplePing:didReceiveUnexpectedPacket:)] ) {
                [strongDelegate pps_simplePing:self didReceiveUnexpectedPacket:packet];
            }
        }
    } else {
...
static void STSocketReadCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    obj = (__bridge PPSSimplePing *) info;
    assert([obj isKindOfClass:[PPSSimplePing class]]);
...
- (void)startWithHostAddress {
    ...
    if (err != 0) {
        ...
        strongDelegate = self.delegate;
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(pps_simplePing:didStartWithAddress:)] ) {
            [strongDelegate pps_simplePing:self didStartWithAddress:self.hostAddress];
        }
    }
    assert(fd == -1);
}
            ...
            if ( address.length >= sizeof(struct sockaddr) ) {
                ...
                switch (addrPtr->sa_family) {
                    case AF_INET: {
                        ...
                        if (self.addressStyle != PPSSimplePingAddressStyleICMPv6) {
                    ...
                    case AF_INET6: {
                        ...
                        self.IPAddress = [NSString stringWithFormat:@"%s", s];
                        if (self.addressStyle != PPSSimplePingAddressStyleICMPv4) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
