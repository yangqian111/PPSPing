...
@protocol PPSSimplePingDelegate;

/*! Controls the IP address version used by SimplePing instances.
 */

typedef NS_ENUM(NSInteger, PPSSimplePingAddressStyle) {
    PPSSimplePingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    PPSSimplePingAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    PPSSimplePingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};
...
@interface PPSSimplePing : NSObject
...
@property (nonatomic, weak, readwrite, nullable) id<PPSSimplePingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) PPSSimplePingAddressStyle addressStyle;
...
@protocol PPSSimplePingDelegate <NSObject>
...
typedef struct PPSICMPHeader PPSICMPHeader;

__Check_Compile_Time(sizeof(PPSICMPHeader) == 8);
__Check_Compile_Time(offsetof(PPSICMPHeader, type) == 0);
__Check_Compile_Time(offsetof(PPSICMPHeader, code) == 1);
__Check_Compile_Time(offsetof(PPSICMPHeader, checksum) == 2);
__Check_Compile_Time(offsetof(PPSICMPHeader, identifier) == 4);
__Check_Compile_Time(offsetof(PPSICMPHeader, sequenceNumber) == 6);

enum {
    PPSICMPv4TypeEchoRequest = 8,          ///< The ICMP `type` for a ping request; in this case `code` is always 0.
    PPSICMPv4TypeEchoReply   = 0           ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};

enum {
    PPSICMPv6TypeEchoRequest = 128,        ///< The ICMP `type` for a ping request; in this case `code` is always 0.
    PPSICMPv6TypeEchoReply   = 129         ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};
